(function ($, window, document, undefined) {
	...
	$.extend(Lightboxed.prototype, {
		init: function () {
			var that = this;
			this.CacheObj =
				jQuery(document).find("#" + namespace + "cache").length > 0 ? jQuery(document).find("#" + namespace + "cache") : jQuery('<div id="' + namespace + 'cache" />').appendTo("body");
			...
			this.lightbox.container =
				jQuery(document).find("#" + namespace + "container").length > 0
					? jQuery(document).find("#" + namespace + "container")
					: jQuery('<div id="' + namespace + 'container" />')
							.addClass(this.options.lightboxCloseClass)
							.appendTo("body");
			this.lightbox.stage =
				this.lightbox.container.find("#" + namespace + "stage").length > 0
					? this.lightbox.container.find("#" + namespace + "stage")
					: jQuery('<div id="' + namespace + 'stage" />').appendTo(this.lightbox.container);
			this.lightbox.content =
				this.lightbox.stage.find("#" + namespace + "content").length > 0
					? this.lightbox.stage.find("#" + namespace + "content")
					: jQuery('<div id="' + namespace + 'content" />').appendTo(this.lightbox.stage);
			this.lightbox.thumbs =
				this.lightbox.container.find("#" + namespace + "thumbs").length > 0
					? this.lightbox.container.find("#" + namespace + "thumbs")
					: jQuery('<div id="' + namespace + 'thumbs" />').appendTo(this.lightbox.stage);
			this.lightbox.close =
				this.lightbox.container.find("#" + namespace + "bttn_close").length > 0
					? this.lightbox.container.find("#" + namespace + "bttn_close")
					: jQuery('<div id="' + namespace + 'bttn_close" />').appendTo(this.lightbox.container);
			this.lightbox.prev =
				this.lightbox.container.find("#" + namespace + "bttn_prev").length > 0
					? this.lightbox.container.find("#" + namespace + "bttn_prev")
					: jQuery('<div id="' + namespace + 'bttn_prev" />').appendTo(this.lightbox.container);
			this.lightbox.next =
				this.lightbox.container.find("#" + namespace + "bttn_next").length > 0
					? this.lightbox.container.find("#" + namespace + "bttn_next")
					: jQuery('<div id="' + namespace + 'bttn_next" />').appendTo(this.lightbox.container);
			this.registerEvents("init");
		},

		addToCache: function (element) {
			if (
				(jQuery(element).attr("data-link") || jQuery(element).attr("href") || jQuery(element).attr("data-src") || jQuery(element).attr("src")).match(
					"jpg|gif|png|JPG|GIF|PNG|JPEG|jpeg|svg|SVG|tif|TIF|bmp|BMP"
				)
			) {
			...
			if (this.Obj.length > 1) {
				jQuery('<div class="' + namespace + 'thumb" />')
					.append(
						jQuery('<div class="' + namespace + 'thumb_border" />').append(
							jQuery("<img />").attr("src", jQuery(element).attr("data-src") || jQuery(element).attr("src") || jQuery(element).attr("data-link") || jQuery(element).attr("href"))
						)
					)
		...
		open: function (event, element) {
			...
			if (this.lightbox.content.children().length > 1 && this.options.loop == true) {
				this.phantoms = {
					left: jQuery(this.lightbox.content.children().get(this.lightbox.content.children().length - 1))
						.clone()
						.addClass(namespace + "phantom-left")
						.prependTo(this.lightbox.content),
					right: jQuery(this.lightbox.content.children().get(1))
						.clone()
						.addClass(namespace + "phantom-right")
						.appendTo(this.lightbox.content),
				};
			}
			this.resizeContent();
		...
		endTouch: function (event, target) {
			...
			event.preventDefault();
			var delta = {
				clientX: touchCoordinates(event).clientX - this.touch.start.clientX,
				clientY: touchCoordinates(event).clientY - this.touch.start.clientY,
				pageX: touchCoordinates(event).pageX - this.touch.start.pageX,
				pageY: touchCoordinates(event).pageY - this.touch.start.pageY,
				timestamp: touchCoordinates(event).timestamp - this.touch.start.timestamp,
			};
		...
		registerEvents: function (scope) {
			...
			if (scope == "lightbox") {
				...
				this.lightbox.next.on("click", function (event) {
					that.next.call(that, event);
				});
				this.lightbox.prev.on("click", function (event) {
					that.prev.call(that, event);
				});
				jQuery(window).on("resize", function (event) {
					that.resizeContent.call(that, event);
				});
				this.lightbox.content.on("touchstart", function (event) {
					that.startTouch.call(that, event, that.lightbox.content);
				});
				this.lightbox.content.on("touchend", function (event) {
					that.endTouch.call(that, event, that.lightbox.content);
				});
				this.lightbox.content.on("touchmove", function (event) {
					that.moveTouch.call(that, event, that.lightbox.content);
				});
			}
			if (scope == "thumbs") {
				...
				this.lightbox.thumbs.children().on("touchend", function (event) {
					that.change.call(that, event, this);
				});
				this.lightbox.thumbs.on("touchstart", function (event) {
					that.startTouch.call(that, event, that.lightbox.thumbs);
				});
				this.lightbox.thumbs.on("touchend", function (event) {
					that.endTouch.call(that, event, that.lightbox.thumbs);
				});
				this.lightbox.thumbs.on("touchmove", function (event) {
					that.moveTouch.call(that, event, that.lightbox.thumbs);
				});
			}
		},
	...
	$.fn.lightboxed = function (options) {
		var createInstance = function () {
			if (!$.data(this, "plugin_" + pluginName)) {
				$.data(this, "plugin_" + pluginName, new Lightboxed(jQuery(this), options));
			}
		};
		var grouped = {};
		jQuery(this)
